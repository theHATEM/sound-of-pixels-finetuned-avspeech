...
def output_visuals(vis_rows, batch_data, outputs, args):
    ...
    for j in range(B):
        ...
        cv2.imwrite(os.path.join(args.vis, filename_mixmag), mix_amp[::-1, :, :][:, :, ::-1])
        cv2.imwrite(os.path.join(args.vis, filename_weight), weight[::-1, :][:, :, ::-1])
        ...
        for n in range(N):
            ...
            cv2.imwrite(os.path.join(args.vis, filename_gtmag), gt_mag[::-1, :, :][:, :, ::-1])
            cv2.imwrite(os.path.join(args.vis, filename_predmag), pred_mag[::-1, :, :][:, :, ::-1])
